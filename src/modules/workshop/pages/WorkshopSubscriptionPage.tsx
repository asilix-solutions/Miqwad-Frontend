 ...
 *
 * Full subscription management page for the workshop provider.
 * Sections (top → bottom):
 *   1. Current-plan hero card — plan name, status pill, price/cycle, days-left
 *      progress bar, start/end/renewedAt dates.
 *   2. Privileges — topListing + freeInspectionOffers with on/off indicators.
 ...
 *
 * Days-left logic and tone thresholds are shared via
 * src/modules/workshop/lib/subscriptionDaysLeft.ts (also used by the dashboard
 * KPI card) so the two views are always consistent.
 */
...
} from "../lib/subscriptionDaysLeft";
import type { WorkshopSubscription } from "../types";
...
function PlanHeroCard({ sub, onRenew, isRenewing }: PlanHeroCardProps) {
  ...
  // Remaining fraction of the total period for the progress bar
  ...
  const renewLabel = isExpiredOrCancelled
    ? t("workshop.subscription.reactivate")
    : t("workshop.subscription.renewBtn");

  const statusLabel = t(`workshop.subscription.status.${sub.status}` as const);
  const cycleSuffix = t(`workshop.subscription.cycleLabel.${sub.billingCycle}` as const);
      ...
      <div className="flex items-start justify-between gap-4 mb-5">
        <div className="min-w-0">
          <p className="text-xs font-medium text-[var(--color-muted)] mb-1">
            {t("workshop.subscription.currentPlan")}
          </p>
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between mb-2">
          <span className="text-xs text-[var(--color-muted)]">
            {t("workshop.subscription.progressLabel")}
          </span>
          <span className="text-sm font-bold" style={{ color: daysDisplayColor }}>
            {daysLeft <= 0
              ? t("workshop.subscription.status.expired")
              : t("workshop.subscription.daysRemaining", { count: Math.max(0, daysLeft) })}
          </span>
        </div>
        ...
        <div
          ...
          aria-valuemin={0}
          aria-valuemax={100}
          aria-label={t("workshop.subscription.daysRemaining", { count: Math.max(0, daysLeft) })}
        >
          <div
      ...
      <div className="flex flex-wrap gap-x-6 gap-y-1.5 text-sm mb-6">
        <span className="text-[var(--color-muted)]">
          <span className="font-medium text-[var(--color-ink-body)]">
            {t("workshop.subscription.startedOn")}
          </span>{" "}
          {formatDate(sub.startDate)}
        </span>
        <span className="text-[var(--color-muted)]">
          <span className="font-medium text-[var(--color-ink-body)]">
            {t("workshop.subscription.expiresOn")}
          </span>{" "}
        ...
        {sub.renewedAt && (
          <span className="text-[var(--color-muted)]">
            <span className="font-medium text-[var(--color-ink-body)]">
              {t("workshop.subscription.renewedOn")}
            </span>{" "}
...
function PrivilegesCard({ sub }: PrivilegesCardProps) {
  ...
  const rows: Array<{ key: keyof typeof sub.privileges; Icon: typeof Star }> = [
    { key: "topListing", Icon: Star },
    { key: "freeInspectionOffers", Icon: Zap },
  ];

  return (
    <ProviderCard>
      <h3 className="text-sm font-semibold text-[var(--color-ink-body)] mb-4">
        {t("workshop.subscription.privileges.title")}
      </h3>
        ...
        {rows.map(({ key, Icon }, idx) => {
          ...
          return (
            ...
            >
              ...
              >
                {t(`workshop.subscription.privileges.${key}` as const)}
              </span>
                ...
                {active ? (
                  <>
                    ...
                    <span
                      ...
                      style={{ color: "var(--color-success-500)" }}
                    >
                      {t("workshop.subscription.privilegeActive")}
                    </span>
                  </>
                ) : (
                  <>
                    ...
                    <span className="text-xs font-medium text-[var(--color-muted)]">
                      {t("workshop.subscription.privilegeInactive")}
                    </span>
                  </>
                )}
              </div>
  ...
  const placeholders = [
    {
      ...
      price: isEn ? "299 SAR / month" : "299 ر.س / شهريًا",
    },
    {
      Icon: Star,
      label: isEn ? "Annual Plan" : "الباقة السنوية",
      price: isEn ? "2,499 SAR / year" : "2,499 ر.س / سنويًا",
    },
  ];
  ...
  return (
    <div>
      <h3 className="text-sm font-semibold text-[var(--color-ink-body)] mb-3">
        {t("workshop.subscription.otherPlans")}
      </h3>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {placeholders.map(({ Icon, label, price }) => (
          <div
            key={label}
            // TODO: wire to plans catalog (backend)
            aria-disabled="true"
          ...
          >
            ...
            <span
              className="inline-flex items-center rounded-[var(--radius-pill)] px-2.5 py-0.5 text-xs font-medium bg-[var(--color-surface-2)] text-[var(--color-muted)]"
            >
              {t("workshop.subscription.comingSoonBadge")}
            </span>
          </div>
        ))}
      </div>
...
function RenewConfirmDialog({
  ...
  return (
    <ProviderDialog
      ...
      title={t("workshop.subscription.renewConfirmTitle")}
      size="sm"
      blurBackdrop
      footer={
        <>
    ...
    >
      <p className="text-sm text-[var(--color-muted)]">
        {t("workshop.subscription.renewConfirmBody")}
      </p>
    </ProviderDialog>
  );
}
  ...
  function handleConfirm() {
    renewMutation.mutate(undefined, {
      onSuccess: () => {
        ...
        toast.success(t("workshop.subscription.renewSuccess"));
      },
      onError: () => {
        toast.error(t("workshop.subscription.renewFailed"));
      },
    });
  }
    ...
    <div className="space-y-6 provider-fade-up">
      ...
      {!subscriptionQuery.isLoading && !subscriptionQuery.isError && !sub && (
        <ProviderEmptyState
          ...
          description={t("workshop.subscription.emptyDescription")}
        />
      )}
