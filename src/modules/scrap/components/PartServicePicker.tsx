/**
 * @file PartServicePicker.tsx
 *
 * Searchable, category-grouped picker for choosing which admin catalog
 * service (`GET /api/Services`) a scrap provider wants to offer as a part.
 * Scrap-framed mirror of the dealer `ServicePicker`, backed by the shared
 * provider-services logic/types — cross-references the scrap provider's
 * existing provider-services rows so already-offered services render
 * disabled with a checkmark.
 *
 * Grouped client-side by each service's first denormalised category (no
 * extra API call — non-admin tokens get 403 on `/api/Categories`). The
 * parent service name (via `parentServiceId`) is shown as a subtle subtitle.
 */
import { useMemo, useState } from "react";
import { useTranslation } from "react-i18next";
import { Check, Package, Search } from "lucide-react";
import { ProviderDialog, ProviderEmptyState, ProviderSkeleton } from "@shared/provider-ui";
import type { ServiceCatalogItem } from "@shared/provider-services";
import { useScrapServiceCatalogQuery } from "../hooks/useScrapPartsQueries";

interface Props {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  /** serviceIds the scrap provider already offers — rendered disabled to prevent duplicates. */
...
export function PartServicePicker({ open, onOpenChange, existingServiceIds, onSelect }: Props) {
  const { t } = useTranslation();
  const catalogQuery = useScrapServiceCatalogQuery();
  ...
  const groups = useMemo(
    () => groupByCategory(filtered, t("scrap.parts.picker.uncategorized")),
    [filtered, t],
  );
  ...
  return (
    <ProviderDialog
      ...
      blurBackdrop
      title={t("scrap.parts.picker.title")}
      description={t("scrap.parts.picker.subtitle")}
      size="md"
    >
      <div className="flex flex-col gap-4">
        ...
        <div className="relative flex items-center">
          ...
          <input
            ...
            placeholder={t("scrap.parts.picker.search")}
          ...
          {!catalogQuery.isLoading && !catalogQuery.isError && groups.length === 0 && (
            <div className="p-4">
              <ProviderEmptyState
                ...
                title={t("scrap.parts.picker.emptyTitle")}
                description={t("scrap.parts.picker.emptyDescription")}
              />
            </div>
          )}
                      ...
                      >
                        ...
                        {isAdded && (
                          <span className="flex shrink-0 items-center gap-1 text-xs font-medium text-[var(--color-success-500)]">
                            ...
                            {t("scrap.parts.picker.alreadyAdded")}
                          </span>
                        )}
                      </button>
                    );
